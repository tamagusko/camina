...
import { checkTimestampSkew, persistDaily } from "@/lib/ingest-store";
...
export async function POST(request: Request, { params }: Ctx) {
  ...
  const skew = checkTimestampSkew(parsed.data.produced_at);
  ...
  // Live mode: idempotent upsert into sensor_daily_totals (H2).
  await persistDaily(parsed.data, id);
  return NextResponse.json({ ok: true, latest_config_version: parsed.data.config_version });
}
