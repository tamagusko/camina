...
- MapLibre GL 6 + OpenFreeMap Positron vector style (no key; needs internet). Protomaps PMTiles is the planned offline/prod basemap. Carto's CDN now watermarks keyless tiles — do not reintroduce it.
...
- **Branches:** `main` (stable, release/hotfix PRs only) · `dev` (integration) · `TRA2026` (frozen paper snapshot). Work branches start from `dev` and PR into `dev`; `dev` → `main` by PR once stable. `dev_expanded_dataset` holds unaudited data — never merge it until the PI has audited it. Details: `CONTRIBUTING.md`.
...
- **Tests:** pytest on edge (165 pass); Vitest 94 unit tests across 11 files (require `data/mock/dublin` fixtures generated by `scripts/generate_mock_dublin.py`; gitignored) + Playwright (3 specs × 2 projects, not run in CI). CI gate: PLAN.md S2.
