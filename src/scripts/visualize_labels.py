...
class CAMINALabelVisualizer:
    ...
    def get_labeled_images(self):
        """Get all images that have corresponding label files"""
        labeled_images = []
    ...
    def visualize_all_continuously(self, method='matplotlib', save_output=False, output_dir=None):
        """Continuously visualize all labeled images"""
...
def main():
    ...
    parser.add_argument("--continuous", "-c", action='store_true',
                       help="Continuously show all labeled images")
    ...
    # Interactive mode - show first few images
        ...
        print("Use --continuous to show all images continuously")

        # Show first 3 images
        labeled_images = visualizer.get_labeled_images()
        for idx, (image_path, labels) in enumerate(labeled_images[:3]):
            visualizer.visualize_single_image(
                image_path.name,
                method=args.method,
                save_output=args.save,
