...
- **Known-fragile areas**: MapLibre canvas sizing race (React Strict Mode disabled as safety net; re-enable when guarded), live read path is a stub (`streets-live.ts`, AUDIT.md G-a), no sensor/token provisioning exists (AUDIT.md G-c), remote config loop is dead (AUDIT.md G-b).
- **Prior decisions locked in the code**: HTTPS-only device transport (MQTT was considered and rejected), Fluid Compute runtime on Vercel, Neon via Marketplace (not self-hosted Postgres), OpenFreeMap basemap (no key; Carto dropped 2026-09-22).
