...
Covers:
  - ``configs/classes.yaml`` is the 9-class canonical set (contiguous, no dupes),
    matching the in-code mirror in ``training/export_ncnn.py``.
  - ``configs/class_mapping.yaml`` is a closed alias table (every
...
def _load_class_taxonomy() -> ModuleType:
    """Import the toolchain loader by file path (training/ is not an installed package)."""
    path = REPO_ROOT / "training" / "class_taxonomy.py"
...
def test_canonical_matches_export_constant() -> None:
    ...
    from training import export_ncnn

    assert ct.load_canonical_classes() == export_ncnn.CAMINAV1_CLASSES
