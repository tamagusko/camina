"""The sensor daemon: counts per 15-minute window, published over HTTPS.

    camera -> detect_and_track -> WindowedCounter -> HttpsPublisher
                                        |                ^
                                        v                |
                                 DailyAccumulator -------+
                                 ConfigPoller (applies server config)

Headless: no window, no frames kept. Run it with ``python -m camina``.
"""
...
class DaemonConfig:
    ...
    # Counting (camina/core/counting.py): count a track when it crosses
...
class SensorDaemon:
    ...
    def __init__(
        ...
        self._daily = DailyAccumulator(db_path=config.state_db_path, classes=config.classes)
    ...
    def _flush_open_window(self) -> None:
        ...
        if snapshot.total() == 0:
            return
        logger.info("Flushing open window on shutdown (%d counts)", snapshot.total())
