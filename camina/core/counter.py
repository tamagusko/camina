...
class WindowedCounter:
    ...
    def force_snapshot(self, now: datetime, *, partial: bool | None = None) -> WindowSnapshot:
    ...
    def _snapshot_and_reset(self, now: datetime, *, partial: bool | None = None) -> WindowSnapshot:
        is_partial = (
...
class DailyAccumulator:
    ...
    def maybe_rollover(self, now: datetime) -> DailySnapshot | None:
        """
        ...
        if published:
            return None
        return DailySnapshot(day=yesterday, totals=totals, window_count=window_count, late=False)
    ...
    def _load(self, day: date) -> tuple[dict[str, int], int, bool] | None:
    ...
    def _save(self, day: date, totals: dict[str, int], window_count: int, published: bool) -> None:
        self._conn.execute(
