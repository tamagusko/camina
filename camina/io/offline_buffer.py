...
class OfflineBuffer:
        ...
        with self._lock:
            ...
            for item in rows:
                ...
                if outcome is SendOutcome.SENT:
                    ...
                    self._poisoned += 1
                    logger.warning(
                        "OfflineBuffer: dropping poison item %d (%s) (total poisoned=%d)",
                        item.id,
                        item.endpoint,
                        self._poisoned,
                    )
    ...
    def stats(self) -> OutboxStats:
        with self._lock:
            ...
            oldest = self._conn.execute("SELECT MIN(enqueued_at) FROM outbox").fetchone()[0]
        return OutboxStats(
            pending=int(pending),
