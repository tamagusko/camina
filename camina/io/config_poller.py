...
from camina.io.http_client import HttpClient
...
class ConfigPoller:
    ...
    def check(self, latest_version: str | None) -> bool:
        ...
        try:
            response = self._http.request("GET", f"/sensors/{self._sensor_id}/config")
        except Exception:
        ...
        try:
            ...
            logger.error("Invalid config payload for sensor %s: %s", self._sensor_id, exc)
            with self._lock:
