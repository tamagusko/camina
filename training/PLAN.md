...
**Scope:** off-Pi training → held-out eval gate (see `training/EVALUATION.md`) →
...
`camina/service/detect_track.py`), the LoRa 9-count codec uses, and the export guard
(`training/export_ncnn.py`) checks:
...
`camina/utils/taxonomy.py:load_canonical_classes`). **Do not reorder or rename** — the index is a
wire contract.
...
| Toolchain 9-class, different names/order (`pedestrian`, `motorcycle`, order ped,cyc,car,moto,bus,truck,escooter,suv,van) | `convert_sdl_to_yolo11.py`, `train_yolo11n.py:111`, `model_comparison_framework.py`, `evaluation_logging_system.py:49-57` | Now consume the SSOT + an **alias table** (`configs/class_mapping.yaml`) via `training/class_taxonomy.py`. `pedestrian`→`person`, `motorcycle`→`motorcyclist`; converter emits canonical order; the comparison per-class mAP is now indexed by canonical order. |
...
| `scripts/train/*.yaml` @ 640, 6-class cyclist datasets | `scripts/train/train_param_warmup.yaml`, `train_param_finetune.yaml` | Left as the two-stage warm-up→fine-tune *pattern*; their `data:` paths still point at old cyclist datasets and must be repointed at the canonical `data.yaml` before use (§0.3). The fuller trainer is `training/train_yolo11n.py`. |

### 0.3 Mapping mechanism (how toolchain names reach canonical)

- **`configs/class_mapping.yaml`** — a checked-in `label-name -> canonical-name`
  ...
  passed through.
- **`training/class_taxonomy.py`** — the loader. `load_canonical_classes()`,
...
- **`training/export_ncnn.py`** reads the same two YAML files independently (kept decoupled
...
1. **Repoint dataset configs — DONE (2026-07-10).** Unzipped `training/SDL
   fine-tuned_v3-cyclist_cleaned.zip` (Roboflow YOLO export: `nc: 6`, `bus, car, cyclist,
   motorcycle, person, truck`; 1224 train + 72 test pairs) into `training/datasets/`
   (gitignored). Ran `convert_sdl_to_yolo11.py` → canonical 9-class dataset at
   `training/datasets/camina_v1_9class/` with `data.yaml` (`nc: 9`, canonical
...
3. **Verify labels — DONE (2026-07-10).** New headless data-time guard
   `training/validate_labels.py` (gap G2): asserts every label's
...
4. **Frozen held-out set — DONE (2026-07-10).** New `training/freeze_holdout.py`
   (`training/EVALUATION.md §1`): deterministic (seed 42) class-presence-stratified carve,
   ...
   `training/holdout_manifest.json` (relative paths + SHA-256 of every image+label +
...
Drive zip (`training/data.md`, `training/SDL fine-tuned_v3-cyclist_cleaned.zip`).

### Ingesting new labelled data

| Step | Tool | Notes |
|---|---|---|
| Convert SDL / Roboflow exports to 9-class YOLO | `training/convert_sdl_to_yolo11.py` | Remaps 6 SDL ids → 9-class schema (`:26-33`). Only classes 0-5 are populated; 6-8 come from labelling (§2). **Fix taxonomy first (§0).** |
...
| Labeler | Path | State |
|---|---|---|
| DINOv3 semi-auto | `training/dinov3_semi_auto_labeling.py` | Sliding-window + **aspect-ratio heuristics** for class (`:211-233`); dummy model fallback (`:88-100`). Suggestions only — not production labels. |
| SAM2 + CLIP | `training/sam2_clip_auto_labeling.py` | SAM2 masks + CLIP text prompts (`:51-79`); random fallback if SAM2/CLIP absent (`:377-379`). Better than DINOv3 but unverified. |
...
### Recipe (recommended path: `training/train_yolo11n.py`)
...
Command:
```bash
uv run python training/train_yolo11n.py \
...
```
candidate best.pt
   │  (1) eval gate — training/EVALUATION.md §4 regression gates
   ▼
   │  (2) NCNN export  ── training/export_ncnn.py --imgsz 640 --half
...
1. **Eval gate.** Candidate must clear the regression gates in `training/EVALUATION.md`
   on the frozen held-out set before anything else.
2. **NCNN export** via `training/export_ncnn.py` (the one hardened, tested export path):
   ```bash
   uv run python -m training.export_ncnn --source models/<date>_caminav1_best.pt --imgsz 640 --half
   ```
   ...
   - The canonical-taxonomy guard (`_verify_canonical_taxonomy`) is the promotion gate — it
     resolves the exported model's `names` through `configs/class_mapping.yaml`
...
4. **Parity check.** NCNN export can shift accuracy vs PyTorch — run the parity check in
   `training/EVALUATION.md §5` on N images before field deployment.
