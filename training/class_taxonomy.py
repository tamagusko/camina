...
``configs/class_mapping.yaml``. This module reads both and provides a
...
Import as ``from training.class_taxonomy import ...``; run the tools from the
repo root as modules (``python -m training.validate_labels``).
The deployment-side export guard (``training/export_ncnn.py``) reads the same
...
def class_mapping_path() -> Path:
    return _project_root() / "configs" / "class_mapping.yaml"


def load_canonical_classes(path: Path | None = None) -> list[str]:
    ...
    if set(raw.keys()) != expected_indices:
        raise TaxonomyError(
            f"{path} indices must be a contiguous 0..{len(raw) - 1} range, got {sorted(raw.keys())}"
        )
...
def load_class_aliases(path: Path | None = None) -> dict[str, str]:
