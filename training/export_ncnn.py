...
3. Canonical taxonomy guard — the exported model's `names`, after alias mapping
   via ``configs/class_mapping.yaml``, must equal the canonical
...
the full deployment context and ``training/PLAN.md`` for the taxonomy /
imgsz contract.
"""
...
from camina.utils.taxonomy import load_canonical_classes, load_class_aliases
from training.pnnx_export import export_torchscript, read_torchscript_metadata, smoke_test
...
def _verify_canonical_taxonomy(names_list: Sequence[str]) -> None:
    ...
    if unmapped:
        raise SystemExit(
            ...
            f"configs/class_mapping.yaml or fix the weights. "
...
def _load_model_meta(source: Path) -> dict | None:
...
def _check_imgsz_contract(source: Path, requested: int) -> None:
    ...
    if meta is None:
        logger.warning(
            ...
            source.with_suffix(".meta.yaml").name,
            source.name,
            requested,
        )
        return
    ...
    if recorded is None or recorded == "unknown" or not isinstance(recorded, int):
        logger.warning(
            "Sidecar for %s records imgsz=%r (unverified); proceeding with imgsz=%d.",
            source.name,
            recorded,
            requested,
        )
