...
class SAM2CLIPAutoLabeler:
        ...
        try:
            ...
            if checkpoint_path and Path(checkpoint_path).exists():
                ...
                sam2_model = build_sam2(
                    "sam2_hiera_large.yaml", checkpoint_path, device=self.device
                )
    ...
    def generate_sam2_masks(self, image: np.ndarray) -> list[dict]:
        ...
        for y in range(step_y // 2, h, step_y):
            for x in range(step_x // 2, w, step_x):
                ...
                try:
                    masks, scores, _logits = self.sam2_model.predict(
    ...
    def _remove_duplicate_masks(self, masks_data: list[dict]) -> list[dict]:
        ...
        for _i, mask_data in enumerate(masks_data):
            is_duplicate = False

            for _j, existing_mask in enumerate(filtered_masks):
    ...
    def process_image(self, image_path: str, output_dir: str | None = None) -> list[BoundingBox]:
        """
    ...
    def apply_nms(self, detections: list[BoundingBox]) -> list[BoundingBox]:
        ...
        # Apply NMS per class
        filtered_detections = []
        for _class_id, class_dets in class_detections.items():
            if not class_dets:
                continue
    ...
    def visualize_detections(
        self, image: np.ndarray, detections: list[BoundingBox], output_path: str | None = None
    ):
        ...
        for det in detections:
            ...
            class_name = self.target_classes.get(det.class_id, {}).get(
                "name", f"class_{det.class_id}"
            )
    ...
    def generate_yolo_labels(self, detections: list[BoundingBox], output_file: str):
        ...
        with open(output_file, "w") as f:
            for det in detections:
                box = f"{det.center_x:.6f} {det.center_y:.6f} {det.width:.6f} {det.height:.6f}"
                line = f"{det.class_id} {box}"
                f.write(line + "\n")
    ...
    def process_dataset(self, image_dir: str, output_dir: str, visualize: bool = True):
        ...
        class_counts = {class_id: 0 for class_id in self.target_classes}
        total_detections = 0
...
def main():
    ...
    parser.add_argument(
        "--output-dir", required=True, help="Output directory for labels and visualizations"
    )
    ...
    labeler = SAM2CLIPAutoLabeler(
        device=args.device, sam2_checkpoint=args.sam2_checkpoint, clip_model=args.clip_model
    )
