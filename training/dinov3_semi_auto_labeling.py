...
class DINOv3SemiAutoLabeler:
    def __init__(self, model_name="dinov2_vits14", device="auto"):
        ...
        self.target_classes = {6: "e-scooter", 7: "SUV", 8: "delivery_van"}
    ...
    def detect_objects_sliding_window(
        self, image: np.ndarray, window_sizes: list[tuple[int, int]], stride_ratio: float = 0.5
    ) -> list[BoundingBox]:
        """
    ...
    def process_image(self, image_path: str, output_dir: str | None = None) -> list[BoundingBox]:
        ...
        detections = [
            d for d in detections if d.width >= self.min_box_size and d.height >= self.min_box_size
        ]
    ...
    def process_dataset(self, image_dir: str, output_dir: str, visualize: bool = True):
        ...
        logger.info(
            f"Processed {len(image_files)} images, found {total_detections} potential objects"
        )
