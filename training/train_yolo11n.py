...
class YOLO11nTrainer:
    ...
    def validate_dataset(self):
        ...
        # configs/class_mapping.yaml, so an alias-named data.yaml
        ...
        if len(actual_classes) != len(expected_classes):
            logger.warning(
                "Class count mismatch. Expected: %d, Got: %d",
...
def main():
    ...
    parser.add_argument(
        "--data", default="all_camina_classes/data.yaml", help="Path to dataset YAML file"
    )
    parser.add_argument("--model", default="yolo11n.pt", help="Path to pretrained model")
    ...
    parser.add_argument(
        "--export", action="store_true", help="Export model for Raspberry Pi after training"
    )
    parser.add_argument(
        "--project", default="camina_expansion", help="Project name for experiment tracking"
    )

    args = parser.parse_args()

    # Initialize trainer
    trainer = YOLO11nTrainer(data_yaml=args.data, model_path=args.model, project_name=args.project)
