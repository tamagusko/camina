...
The export guard (``training/export_ncnn.py``) only covers export time.
...
from training.class_taxonomy import (
    TaxonomyError,
...
def _names_in_order(names: object) -> list[str]:
    ...
    raise LabelValidationError(
        f"data.yaml:names must be a dict or list, got {type(names).__name__}"
    )
...
def _validate_label_line(line: str) -> tuple[int, str]:
    ...
    for name, raw in zip(("cx", "cy", "w", "h"), parts[1:5], strict=False):
        try:
            value = float(raw)
        except ValueError:
